...
powerfactor_raw <- read.csv("data/pf.csv")
voltage_raw <- read.csv("data/voltagereads.csv")
...
# Calculate the total MVARs which is the total used or 
...
```{r Summarise powerfactor_tidy to the hour - summary_pf}
# Some stations will not have data in powerfactor_tidy
# I think this contains the same values as powerfactor_tidy
# needs a review
summary_pf <- powerfactor_tidy %>%
  group_by(substationName, dtReadDay, h, desc)  %>%
  summarise( pf.min = min(pf),            
             pf.mean = mean(pf),
             pf.median = median(as.numeric(pf)),
             pf.std.dev = sd(pf),
             pf.varience = var(pf),
             pf.max = max(pf), 
             pf.sem = sd(pf)/sqrt(length(pf)),
             mw  = sum(mw),
             mvar  = sum(mvar),
             pfc.min = min(pfChart),            
             pfc.mean = mean(pfChart),
             pfc.median = median(as.numeric(pfChart)),
             pfc.max = max(pfChart), 
             pf.n = n())  %>% 
  arrange(substationName, dtReadDay, h, desc)
```

```{r Create summary_total from Merging summary_pf and summary_voltage to the hour}
...
# Notes, I need to remove desc because the shifted value
# for the pfc (power factor chart value), will plot
# the value correctly on the chart without needing the 
# desc categorical value (lagging and leading)
...
```{r Univariate_Plots Voltage Facet hour and substation, fig.width=12, fig.height=12}
ggplot(data = voltage_tidy,aes(x = VoltsHalf)) + 
  geom_histogram(binwidth=0.5, color = 'black', fill = '#333333') + 
  theme_bw(base_family= 'Helvetica') +
...
```{r Univariate_Plots Voltage Facet hour and substation free scales, fig.width=12, fig.height=12}
ggplot(data = voltage_tidy,aes(x = VoltsHalf)) + 
  geom_histogram(binwidth=0.5, color = 'black', fill = '#333333') + 
  theme_bw(base_family= 'Helvetica') +
...
#### Power Factor
The power factor dataset contains 10,800 obs. of 4 variables which are readings
...
`r str(powerfactor_raw)`

#### Voltage  
The power factor dataset contains 3,033,369 obs. of 4 variables which are 
...
willamsport <-
  ggplot(data = subset(voltage_tidy, substationName=="WILLIAMSPORT"),aes(x = VoltsHalf)) + 
  geom_histogram(binwidth=0.5, color = 'black', fill = '#333333') + 
  theme_bw(base_family= 'Helvetica') +
  ggtitle("WILLIAMSPORT (bin=0.5)") +
  xlab("voltage")

mtpleasant <-ggplot(data = subset(voltage_tidy, substationName=="MT PLEASANT"),aes(x = VoltsHalf)) + 
  geom_histogram(binwidth=0.5, color = 'black', fill = '#333333') + 
  theme_bw(base_family= 'Helvetica') +
  ggtitle("MT PLEASANT (bin=0.5)") +
  xlab("voltage")

grid.arrange(willamsport, mtpleasant, ncol=2)
...
ggplot(data=voltage_tidy[!is.na(voltage_tidy$VoltsHalf), ], 
    ...
    geom_hline(yintercept = voltageLow, color='blue') +
    theme(axis.text.x=element_text(angle = -90, hjust = 0)) +
    ggtitle("Boxplot of Voltage by hour for Substations" ) 
...
#   theme_bw(base_family= 'Helvetica')  +
#   theme(axis.text.x = element_text(angle = -90, hjust = 0)) +
#   ggtitle("Voltage by Time (hour:min)" )

ggplot(data=subset(voltage_tidy,substationName == "BOYNTON VALLEY"), aes(x=hm, y=VoltsHalf)) +
  geom_point(alpha=1/5) + 
  facet_wrap(~ dtReadDay, ncol = 3) +
  scale_x_discrete(breaks=c("00:00","06:00","12:00","18:00","23:00")) +
  xlab("hr:min") +
  ...
  theme_bw(base_family= 'Helvetica')  +
  theme(axis.text.x = element_text(angle = -90, hjust = 0)) +
  ggtitle("BOYNTON VALLEY Voltage by Time (hour:min)" )
...
ggplot(data=powerfactor_tidy[!is.na(powerfactor_tidy$mw), ], aes(x=h, y=mw)) +
  geom_point() + 
...
meters114 <- subset(substation_meter_review_114, count>4)
meters <- voltage_tidy[(voltage_tidy$meter %in% meters114$meter),]

ggplot(data=subset(meters, substationName=='CHAPEL HILL'), 
       aes(x=hm, y=VoltsHalf, color=substationName)) +
  geom_point() + 
...
##### Voltage Bucket Data
```{r Summary Of Voltage Bucket Table Data By Substation - substation_bucket_review table}
# output to table
kable(substation_bucket_review)
...
# Models
Predict Power Factor... is the goal
...
heatmap_voltage_15min +
 ...
 scale_fill_gradient2(
                      midpoint=mean(voltage_tidy$VoltsHalf),
                      limits=c(min(voltage_tidy$VoltsHalf)
                               ,max(voltage_tidy$VoltsHalf))) +
 theme_bw(base_family= 'Helvetica')  +
...
heatmap_voltage_hour_min + geom_tile(colour="white") +
 scale_fill_gradient2(
