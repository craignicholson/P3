...
and power factor as well as megawatts and megavars and to review the relatioship
between power factor and voltage.
...
VARh, volts.  An utility needs to maintain a specified range for voltage across
...
is normally considered the generator."[1]
...
usually charge a higher cost to industrial or commercial customers where there 
is a low power factor." [1]
...
in the circuit would be doubled." [1]
...
affect the efficiency of power transmission." [1]
...
# The lowest common denominator we will support is hour
...
#   mwsquared
#   mvarsquared
#   desc (Lagging or Leading) - the sign of the phase angle
#   hm (hour:min)
...
# rename the columns
# TODO: If the pivot cols are missing code fails, need to check for empty data frame
...
```{r Power Factor calculate MVARs}
...
```{r Power Factor - Calculate Power Factor and VoltAmpere}
# VA =  sqrt((powerfactor$mw^2 + powerfactor$mvar^2))
# VA = (mw^2 + (mvar)^2) 
# this is what we have to generate to provide the mwatts
powerfactor$va <-  sqrt((powerfactor$mw^2 + powerfactor$mvar^2))

# Calculate the power factor using formuals below - Ratio
...
```{r Univariate_Plots Voltage Hist Facet Substation free scales, fig.width=12, fig.height=height}
ggplot(data = voltage_tidy,aes(x = VoltsHalf)) + 
  geom_histogram(binwidth=0.5, color = 'black', fill = '#333333') + 
  theme_bw(base_family= 'Helvetica') +
...
```{r Univariate_Plots Voltage Hist Facet by Date for Austin free scales, fig.width=12, fig.height=height}
ggplot(data = voltage_tidy[voltage_tidy$substationName =='AUSTIN',],aes(x = VoltsHalf)) + 
  geom_histogram(binwidth=0.5, color = 'black', fill = '#333333') + 
  theme_bw(base_family= 'Helvetica') +
  facet_wrap(~dtReadDay, ncol = 4, scales = 'free') +
  ggtitle(paste("AUSTIN - Histogram of Voltage By Day (bin=0.5)\n",
                as.character(length(unique(voltage_tidy$dtReadDate))),
                "unique intervals")) +
  xlab("voltage")
```
...
```{r Univariate_Plots histogram MW Facet Hour and Substation, fig.width=12, fig.height=12}
ggplot(data = powerfactor_tidy,aes(x = mw)) + 
  geom_histogram(binwidth=0.25, color = 'black', fill = '#333333') + 
  theme_bw(base_family= 'Helvetica') +
...
austin + facet_wrap(~dtReadDay)
dallas + facet_wrap(~dtReadDay)
...
ggplot(data=voltage_tidy[!is.na(voltage_tidy$VoltsHalf), ], 
    ...
    theme(legend.position="bottom") +
    xlab("") +
    geom_hline(yintercept = voltageHigh, color='blue',alpha=0.5) +
    geom_hline(yintercept = voltageLow, color='blue',alpha=0.5) +
...
```{r ROCKWALL Meter Load Profiles for Voltage}
# most varience
rockwall <- subset(voltage_tidy,substationName == "ROCKWALL")
t <- ggplot(data=subset(rockwall,!is.na(VoltsHalf)), aes(x=hm, y=VoltsHalf)) +
  geom_line(alpha=1/1, color=factor(rockwall$meter)) + 
  facet_wrap(~ dtReadDay, ncol = 3) +
  scale_x_discrete(breaks=c("00:00","06:00","12:00","18:00","23:00")) +
  xlab("hr:min") +
  ylab("voltage") +
  theme_bw(base_family= 'Helvetica')  +
  theme(axis.text.x = element_text(angle = -90, hjust = 0)) +
  ggtitle(paste("ROCKWALL Voltage by Time (hour:min)\nmeters:", 
                as.character(length(unique(rockwall$meter))))) 


# grid.text(paste("Variance: ", as.character(var(rockwall$VoltsHalf))))
# ggdraw(add_sub(t, "This is an annotation.\nAnnotations can span multiple lines."))

```


```{r HILLSBORO Meter Load Profiles for Voltage}
# least varience 
hillsboro <- subset(voltage_tidy,substationName == "HILLSBORO")
ggplot(data=subset(hillsboro,!is.na(VoltsHalf)), aes(x=hm, y=VoltsHalf)) +
  geom_line(alpha=1/1, color=factor(hillsboro$meter)) + 
  facet_wrap(~ dtReadDay, ncol = 3) +
  scale_x_discrete(breaks=c("00:00","06:00","12:00","18:00","23:00")) +
  xlab("hr:min") +
  ylab("voltage") +
  ...
  theme(axis.text.x = element_text(angle = -90, hjust = 0)) +
  ggtitle(paste("ROCKWALL Voltage by Time (hour:min)\nmeters:", 
                as.character(length(unique(hillsboro$meter)))))

var(hillsboro$VoltsHalf)
```
...
```{r Voltage Bucket Summary - GOOD ONE}
summary(voltage_tidy$voltage.bucket)

substation_bucket_day_hour <-
  voltage_tidy %>% 
    group_by(dtReadDay, substationName, h, voltage.bucket) %>% 
    summarise(count = n())  %>% 
        arrange(dtReadDay, substationName, h, voltage.bucket)
...
ggplot(aes(x = h, y = count),
       data = subset(substation_bucket_day_hour, !is.na(voltage.bucket))) +
       geom_line(aes(color=voltage.bucket), stat='summary', fun.y=mean) +
       geom_line(stat = 'summary' , fun.y = mean, linetype=2) +
       facet_wrap(~substationName, scales = "free_y") +
       ggtitle("Median Voltage Counts By Hour")


ggplot(aes(x = h, y = count),
       data = subset(substation_bucket_day_hour, 
                     !is.na(voltage.bucket) & 
                     substationName =="DALLAS")) +
       geom_line(aes(color=voltage.bucket), stat='summary', fun.y=mean) +
       geom_line(stat = 'summary' , fun.y = mean, linetype=2) +
       facet_wrap(~dtReadDay, scales = "free_y") +
       ggtitle("Median Voltage Counts By Hour\n Dallas Substation")
...
#summary(subset(powerfactor_tidy, substationName=="DALLAS" & dtReadDay==""))
summary(subset(powerfactor_tidy, substationName=="DALLAS"))

```

```{r POWER FACTOR BUCKETS RANGE}
ggplot(aes(x=h, y=pf), data=subset(powerfactor_tidy)) +
    geom_line(aes(color=pf.range), stat='summary', fun.y=mean) +
    geom_line(stat = 'summary' , fun.y = mean, linetype=2) +
    ggtitle("power factor vs hour")  +
    geom_line(aes(color = pf.range))
...
I was interested in finding substations where the voltage has high varience and
to see what affect this has on the power factor.  When reviewing this data
day to day variations are affected by the voltage requlators to help reduce the
voltage on the transmission lines.  A voltage load profile with less varience
should have a better power factor (what does better mean, 0.98-1, lagging or leading?[2]

Identify which days have VVC (Volt Var Contol On).
3/2 off 
3/3 On
--- Walter Has these notes.
...
The section provides a rich and well-written reflection of 
  * struggles
    * Date Formatting without lubridate was one mistake or line of code 
    which would not run.
    * Exploring for relationships - In industry standards it is typically regarded
    as controling voltage with less varience helps keep the power factor with in
    nominal ranges.  When looking for the correlation between power factor and
    votlage we really can not see one most likely because the voltage is in
    a narrow range, and power factor is more affected by kw and mvars since it
    is a ratio of real / active power.
...
The section poses ideas or questions for future work. 
  ...
  * examining the varience of all the load shapes for each
  variable.
  * revewing the load shape for all meters on every substation.
...
S, Apparent Power    (volt amperess)
...
[2] https://en.wikipedia.org/wiki/Distribution_management_system#Volt-VAR_Control_.28VVC.29
...
qwerty <- ggplot(data=subset(powerfactor_tidy, substationName=="DALLAS"), aes(x=h)) +
  geom_point(aes(y=mw,colour="mw")) +
  geom_line(linetype=2, aes(y=va, colour="va")) +
  geom_line(aes(y=mvar.delivered, colour="mvar.delivered")) +
  geom_line(aes(y=mvar.received, colour="mvar.received")) +
  scale_colour_manual("", 
                      breaks = c("mw", "va", "mvar.delivered", "mvar.received"),
                      values = c("mw"="red", 
                                 "va"="orange", 
                                 "mvar.delivered"="yellow", 
                                 "mvar.received"="blue", 
                                 "pf"="black")) +
  scale_y_continuous(limits= c(-5, 15)) +
  facet_wrap(substationName~dtReadDay, ncol=3, scales = "free_x") +
  ylab("power factor") +
  theme_bw(base_family= 'Helvetica') 

ggplot(data=subset(powerfactor_tidy, substationName=="DALLAS"), aes(x=h))+
  geom_point(aes(y=mw,colour="mw")) +
  geom_line(linetype=2, aes(y=va, colour="va")) +
  geom_line(aes(y=mvar.delivered, colour="mvar.delivered")) +
  geom_line(aes(y=mvar.received, colour="mvar.received")) +
  geom_line(aes(y=pfChart), colour='black') +
  facet_grid(.~dtReadDay,scales="free_y") +
  scale_colour_manual("", 
                      breaks = c("mw", "va", "mvar.delivered", "mvar.received","pfChart"),
                      values = c("mw"="green", 
                                 "va"="blue", 
                                 "mvar.delivered"="orange", 
                                 "mvar.received"="violet", 
                                 "pfChart"="black"))  
